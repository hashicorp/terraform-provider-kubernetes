...
func flattenPodAffinityTerms(in []v1.PodAffinityTerm) []interface{} {
	...
	for i, n := range in {
		m := make(map[string]interface{})
		m["namespaces"] = structures.NewStringSet(schema.HashString, n.Namespaces)
		m["topology_key"] = n.TopologyKey
		if n.LabelSelector != nil {
			m["label_selector"] = structures.FlattenLabelSelector(n.LabelSelector)
		}
		att[i] = m
	}
	return att
...
func expandPodAffinityTerms(t []interface{}) []v1.PodAffinityTerm {
	...
	for i, n := range t {
		...
		if v, ok := in["label_selector"].([]interface{}); ok && len(v) > 0 {
			obj[i].LabelSelector = structures.ExpandLabelSelector(v)
		}
		if v, ok := in["namespaces"].(*schema.Set); ok {
			obj[i].Namespaces = structures.SliceOfString(v.List())
		}
