...
type CronJobModel struct {
	...
	Spec struct {
		ConcurrencyPolicy      types.String `tfsdk:"concurrency_policy" manifest:"concurrencyPolicy"`
		FailedJobsHistoryLimit types.Int64  `tfsdk:"failed_jobs_history_limit" manifest:"failedJobsHistoryLimit"`
		JobTemplate            struct {
			Metadata struct {
				Annotations                map[string]types.String `tfsdk:"annotations" manifest:"annotations"`
			...
			Spec struct {
				...
				BackoffLimit          types.Int64  `tfsdk:"backoff_limit" manifest:"backoffLimit"`
				BackoffLimitPerIndex  types.Int64  `tfsdk:"backoff_limit_per_index" manifest:"backoffLimitPerIndex"`
				CompletionMode        types.String `tfsdk:"completion_mode" manifest:"completionMode"`
				Completions           types.Int64  `tfsdk:"completions" manifest:"completions"`
				ManualSelector        types.Bool   `tfsdk:"manual_selector" manifest:"manualSelector"`
				MaxFailedIndexes      types.Int64  `tfsdk:"max_failed_indexes" manifest:"maxFailedIndexes"`
				Parallelism           types.Int64  `tfsdk:"parallelism" manifest:"parallelism"`
				PodFailurePolicy      struct {
					Rules []struct {
						Action      types.String `tfsdk:"action" manifest:"action"`
						OnExitCodes struct {
							...
							Values        []types.Int64 `tfsdk:"values" manifest:"values"`
						} `tfsdk:"on_exit_codes" manifest:"onExitCodes"`
						OnPodConditions []struct {
							Status types.String `tfsdk:"status" manifest:"status"`
							Type   types.String `tfsdk:"type" manifest:"type"`
						} `tfsdk:"on_pod_conditions" manifest:"onPodConditions"`
					} `tfsdk:"rules" manifest:"rules"`
				} `tfsdk:"pod_failure_policy" manifest:"podFailurePolicy"`
				PodReplacementPolicy types.String `tfsdk:"pod_replacement_policy" manifest:"podReplacementPolicy"`
				Selector             struct {
				...
				Suspend  types.Bool `tfsdk:"suspend" manifest:"suspend"`
				Template struct {
					Metadata struct {
						Annotations                map[string]types.String `tfsdk:"annotations" manifest:"annotations"`
						CreationTimestamp          types.String            `tfsdk:"creation_timestamp" manifest:"creationTimestamp"`
				...
				TtlSecondsAfterFinished types.Int64 `tfsdk:"ttl_seconds_after_finished" manifest:"ttlSecondsAfterFinished"`
			} `tfsdk:"spec" manifest:"spec"`
		} `tfsdk:"job_template" manifest:"jobTemplate"`
		Schedule                   types.String `tfsdk:"schedule" manifest:"schedule"`
		StartingDeadlineSeconds    types.Int64  `tfsdk:"starting_deadline_seconds" manifest:"startingDeadlineSeconds"`
		SuccessfulJobsHistoryLimit types.Int64  `tfsdk:"successful_jobs_history_limit" manifest:"successfulJobsHistoryLimit"`
		Suspend                    types.Bool   `tfsdk:"suspend" manifest:"suspend"`
		TimeZone                   types.String `tfsdk:"time_zone" manifest:"timeZone"`
	} `tfsdk:"spec" manifest:"spec"`
}
