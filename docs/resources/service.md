...
- `traffic_distribution` (String) Specifies the preferred strategy for distributing traffic to Service endpoints. Default to K8s default routing strategy when not set. Valid options are `PreferSameZone`, `PreferSameNode`, `PreferClose` (deprecated since K8s 1.35).
