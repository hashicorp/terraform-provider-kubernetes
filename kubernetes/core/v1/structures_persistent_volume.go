...
func flattenCephFSVolumeSource(in *v1.CephFSVolumeSource) []interface{} {
	att := make(map[string]interface{})
	att["monitors"] = structures.NewStringSet(schema.HashString, in.Monitors)
	if in.Path != "" {
		att["path"] = in.Path
	}
	if in.User != "" {
...
func flattenFCVolumeSource(in *v1.FCVolumeSource) []interface{} {
	att := make(map[string]interface{})
	att["target_ww_ns"] = structures.NewStringSet(schema.HashString, in.TargetWWNs)
	att["lun"] = *in.Lun
	if in.FSType != "" {
		att["fs_type"] = in.FSType
	}
...
func flattenRBDVolumeSource(in *v1.RBDVolumeSource) []interface{} {
	att := make(map[string]interface{})
	att["ceph_monitors"] = structures.NewStringSet(schema.HashString, in.CephMonitors)
...
func expandAzureDiskVolumeSource(l []interface{}) *v1.AzureDiskVolumeSource {
	...
	if v, ok := in["fs_type"].(string); ok {
		obj.FSType = structures.PtrToString(v)
	}
	if v, ok := in["read_only"].(bool); ok {
		obj.ReadOnly = structures.PtrToBool(v)
	}
...
func expandCephFSVolumeSource(l []interface{}) *v1.CephFSVolumeSource {
	...
	obj := &v1.CephFSVolumeSource{
		Monitors: structures.SliceOfString(in["monitors"].(*schema.Set).List()),
	}
...
func expandFCVolumeSource(l []interface{}) *v1.FCVolumeSource {
	...
	obj := &v1.FCVolumeSource{
		TargetWWNs: structures.SliceOfString(in["target_ww_ns"].(*schema.Set).List()),
		Lun:        structures.PtrToInt32(int32(in["lun"].(int))),
	}
	if v, ok := in["fs_type"].(string); ok {
		obj.FSType = v
	}
...
func expandFlexPersistentVolumeSource(l []interface{}) *v1.FlexPersistentVolumeSource {
	...
	if v, ok := in["options"].(map[string]interface{}); ok && len(v) > 0 {
		obj.Options = structures.ExpandStringMap(v)
	}
	return obj
}
...
func expandPersistentVolumeSpec(l []interface{}) (*v1.PersistentVolumeSpec, error) {
	...
	if v, ok := in["mount_options"].(*schema.Set); ok && v.Len() > 0 {
		obj.MountOptions = structures.SchemaSetToStringArray(v)
	}
...
func expandCSIPersistentDiskVolumeSource(l []interface{}) *v1.CSIPersistentVolumeSource {
	...
	if v, ok := in["volume_attributes"].(map[string]interface{}); ok && len(v) > 0 {
		obj.VolumeAttributes = structures.ExpandStringMap(v)
	}
...
func expandRBDVolumeSource(l []interface{}) *v1.RBDVolumeSource {
	...
	obj := &v1.RBDVolumeSource{
		CephMonitors: structures.ExpandStringSlice(in["ceph_monitors"].(*schema.Set).List()),
...
func patchPersistentVolumeSpec(pathPrefix, prefix string, d *schema.ResourceData) (structures.PatchOperations, error) {
...
func patchPersistentVolumeSource(pathPrefix, prefix string, d *schema.ResourceData) []structures.PatchOperation {
	ops := make([]structures.PatchOperation, 0)
	...
	if d.HasChange(prefix + "gce_persistent_disk") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/gcePersistentDisk"})
		}
	}
	...
	if d.HasChange(prefix + "aws_elastic_block_store") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/awsElasticBlockStore"})
		}
	}
	...
	if d.HasChange(prefix + "host_path") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/hostPath"})
		}
	}
	...
	if d.HasChange(prefix + "glusterfs") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/glusterfs"})
		}
	}
	...
	if d.HasChange(prefix + "nfs") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/nfs"})
		}
	}
	...
	if d.HasChange(prefix + "rbd") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/rbd"})
		}
	}
	...
	if d.HasChange(prefix + "iscsi") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/iscsi"})
		}
	}
	...
	if d.HasChange(prefix + "cinder") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/cinder"})
		}
	}
	...
	if d.HasChange(prefix + "ceph_fs") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/cephfs"})
		}
	}
	...
	if d.HasChange(prefix + "fc") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/fc"})
		}
	}
	...
	if d.HasChange(prefix + "flocker") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/flocker"})
		}
	}
	...
	if d.HasChange(prefix + "flex_volume") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/flexVolume"})
		}
	}
	...
	if d.HasChange(prefix + "azure_file") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/azureFile"})
		}
	}
	...
	if d.HasChange(prefix + "vsphere_volume") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/vsphereVolume"})
		}
	}
	...
	if d.HasChange(prefix + "quobyte") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/quobyte"})
		}
	}
	...
	if d.HasChange(prefix + "azure_disk") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/azureDisk"})
		}
	}
	...
	if d.HasChange(prefix + "photon_persistent_disk") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: pathPrefix + "/photonPersistentDisk"})
		}
	}
	...
	if d.HasChange(prefix + "csi") {
		...
		if newOk && len(newV) > 0 {
			...
			ops = append(ops, &structures.RemoveOperation{Path: path})
		}
	}
