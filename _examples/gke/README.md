...
This example generates a kubeconfig file in the current working directory. However, the token in this config will expire after 1 hour. I can be refreshed by running `terraform apply` again. Alternatively, a longer-lived configuration can be generated using the gcloud tool.

```
gcloud container clusters get-credentials $(terraform output cluster_name) --region $GOOGLE_REGION
```
