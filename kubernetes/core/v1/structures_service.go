...
func flattenServiceSpec(in v1.ServiceSpec) []interface{} {
	...
	if len(in.LoadBalancerSourceRanges) > 0 {
		att["load_balancer_source_ranges"] = structures.NewStringSet(schema.HashString, in.LoadBalancerSourceRanges)
	}
	if in.ExternalName != "" {
...
func expandServiceSpec(l []interface{}) v1.ServiceSpec {
	...
	if v, ok := in["selector"].(map[string]interface{}); ok && len(v) > 0 {
		obj.Selector = structures.ExpandStringMap(v)
	}
	if v, ok := in["cluster_ip"].(string); ok {
		obj.ClusterIP = v
	}
	if v, ok := in["cluster_ips"].([]interface{}); ok && len(v) > 0 {
		obj.ClusterIPs = structures.ExpandStringSlice(v)
	}
	...
	if v, ok := in["external_ips"].(*schema.Set); ok && v.Len() > 0 {
		obj.ExternalIPs = structures.SliceOfString(v.List())
	}
	...
	if v, ok := in["load_balancer_source_ranges"].(*schema.Set); ok && v.Len() > 0 {
		obj.LoadBalancerSourceRanges = structures.SliceOfString(v.List())
	}
...
func patchServiceSpec(keyPrefix, pathPrefix string, d *schema.ResourceData, kv *gversion.Version) (structures.PatchOperations, error) {
