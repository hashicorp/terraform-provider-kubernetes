...
// sdkv2providerMeta configures the SDKv2 provider and returns its Meta func, which
// the Framework server reuses as the shared Kubernetes client (see provider_configure.go).
...
// kubernetes_manifest_yaml) over protocol v6, wired to the shared SDKv2 meta.
var testAccProtoV6ProviderFactories = map[string]func() (tfprotov6.ProviderServer, error){
	"kubernetes": providerserver.NewProtocol6WithError(provider.New("test", sdkv2providerMeta())),
}

// testAccClients returns the shared Kubernetes client set for Exists/Destroy checks.
