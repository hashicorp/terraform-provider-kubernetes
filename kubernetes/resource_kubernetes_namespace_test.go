...
// These tests cover the DEPRECATED kubernetes_namespace alias, which is still
// served by the SDKv2 implementation in resource_kubernetes_namespace_v1.go.
//
// kubernetes_namespace_v1 itself moved to the Plugin Framework
// (internal/framework/provider/corev1), and its own tests moved with it. The
// scenarios here are the ones that used to exercise kubernetes_namespace_v1,
// re-pointed rather than deleted: the same Go constructor still backs the alias,
// so deleting them would have left the shipping SDKv2 code path with no coverage
// at all.
...
func TestAccKubernetesNamespace_identity(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccKubernetesNamespaceConfig_basic(name),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckKubernetesNamespaceExists(resourceName, &nsConf),
...
func testAccCheckKubernetesNamespaceExists(n string, obj *corev1.Namespace) resource.TestCheckFunc {
