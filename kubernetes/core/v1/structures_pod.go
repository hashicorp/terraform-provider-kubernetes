...
func flattenPodSecurityContext(in *v1.PodSecurityContext) []interface{} {
	...
	if len(in.SupplementalGroups) > 0 {
		att["supplemental_groups"] = structures.NewInt64Set(schema.HashSchema(&schema.Schema{
			Type: schema.TypeInt,
...
func flattenTopologySpreadConstraints(tsc []v1.TopologySpreadConstraint) []interface{} {
	...
	for _, v := range tsc {
		...
		if v.LabelSelector != nil {
			obj["label_selector"] = structures.FlattenLabelSelector(v.LabelSelector)
		}
		att = append(att, obj)
	}
	return att
...
func ExpandPodSpec(p []interface{}) (*v1.PodSpec, error) {
	...
	if v, ok := in["automount_service_account_token"].(bool); ok {
		obj.AutomountServiceAccountToken = structures.PtrToBool(v)
	}
	...
	if v, ok := in["enable_service_links"].(bool); ok {
		obj.EnableServiceLinks = structures.PtrToBool(v)
	}
	...
	if v, ok := in["share_process_namespace"]; ok {
		obj.ShareProcessNamespace = structures.PtrToBool(v.(bool))
	}
	...
	if v, ok := in["termination_grace_period_seconds"].(int); ok {
		obj.TerminationGracePeriodSeconds = structures.PtrToInt64(int64(v))
	}
...
func expandPodDNSConfig(l []interface{}) (*v1.PodDNSConfig, error) {
	...
	if v, ok := in["nameservers"].([]interface{}); ok {
		obj.Nameservers = structures.ExpandStringSlice(v)
	}
	if v, ok := in["searches"].([]interface{}); ok {
		obj.Searches = structures.ExpandStringSlice(v)
	}
...
func expandDNSConfigOptions(options []interface{}) ([]v1.PodDNSConfigOption, error) {
	...
	for i, c := range options {
		...
		if v, ok := in["value"].(string); ok {
			opt.Value = structures.PtrToString(v)
		}
		opts[i] = opt
	}
...
func expandPodSecurityContext(l []interface{}) (*v1.PodSecurityContext, error) {
	...
	if v, ok := in["fs_group"].(string); ok && v != "" {
		...
		obj.FSGroup = structures.PtrToInt64(int64(i))
	}
	if v, ok := in["run_as_group"].(string); ok && v != "" {
		...
		obj.RunAsGroup = structures.PtrToInt64(int64(i))
	}
	if v, ok := in["run_as_non_root"].(bool); ok {
		obj.RunAsNonRoot = structures.PtrToBool(v)
	}
	if v, ok := in["run_as_user"].(string); ok && v != "" {
		...
		obj.RunAsUser = structures.PtrToInt64(int64(i))
	}
	...
	if v, ok := in["supplemental_groups"].(*schema.Set); ok {
		obj.SupplementalGroups = structures.SchemaSetToInt64Array(v)
	}
...
func expandKeyPath(in []interface{}) []v1.KeyToPath {
	...
	for i, c := range in {
		...
		if v, ok := p["mode"].(string); ok {
			...
			if err == nil {
				keyPaths[i].Mode = structures.PtrToInt32(int32(m))
			}
		}
...
func expandDownwardAPIVolumeFile(in []interface{}) ([]v1.DownwardAPIVolumeFile, error) {
	...
	for i, c := range in {
		...
		if mode, ok := p["mode"].(string); ok && len(mode) > 0 {
			...
			dapivf[i].Mode = structures.PtrToInt32(int32(m))
		}
		if v, ok := p["path"].(string); ok {
			dapivf[i].Path = v
		}
...
func expandConfigMapVolumeSource(l []interface{}) (*v1.ConfigMapVolumeSource, error) {
	...
	if mode, ok := in["default_mode"].(string); ok && len(mode) > 0 {
		...
		obj.DefaultMode = structures.PtrToInt32(int32(v))
	}
	...
	if opt, ok := in["optional"].(bool); ok {
		obj.Optional = structures.PtrToBool(opt)
	}
...
func expandProjectedSecret(secret map[string]interface{}) *v1.SecretProjection {
	...
	if value, ok := secret["optional"].(bool); ok {
		s.Optional = structures.PtrToBool(value)
	}
	return s
}
...
func expandProjectedServiceAccountToken(sat map[string]interface{}) (*v1.ServiceAccountTokenProjection, error) {
	...
	if value, ok := sat["expiration_seconds"].(int); ok {
		s.ExpirationSeconds = structures.PtrToInt64(int64(value))
	}
...
func expandTolerations(tolerations []interface{}) ([]*v1.Toleration, error) {
	...
	for i, t := range tolerations {
		...
		if value, ok := m["toleration_seconds"].(string); ok && value != "" {
			...
			ts[i].TolerationSeconds = structures.PtrToInt64(seconds)
		}
...
func expandTopologySpreadConstraints(tsc []interface{}) ([]*v1.TopologySpreadConstraint, error) {
	...
	for i, t := range tsc {
		...
		if v, ok := m["label_selector"].([]interface{}); ok && len(v) > 0 {
			ts[i].LabelSelector = structures.ExpandLabelSelector(v)
		}
...
func patchPodSpec(pathPrefix, prefix string, d *schema.ResourceData) (structures.PatchOperations, error) {
