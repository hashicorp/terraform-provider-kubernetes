...
func resourceKubernetesPersistentVolumeClaimCreate(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	if d.Get("wait_until_bound").(bool) {
		...
		if err != nil {
			...
			lastWarnings, wErr = structures.GetLastWarningsForObject(ctx, conn, out.ObjectMeta, "PersistentVolumeClaim", 3)
			if wErr != nil {
				return diag.FromErr(wErr)
			}

			if len(lastWarnings) == 0 {
				lastWarnings, wErr = structures.GetLastWarningsForObject(ctx, conn, metav1.ObjectMeta{
