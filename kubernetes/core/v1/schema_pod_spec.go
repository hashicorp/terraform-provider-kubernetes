...
func PodSpecFields(isUpdatable, isComputed bool) map[string]*schema.Schema {
	s := map[string]*schema.Schema{
		...
		"dns_policy": {
			...
			Computed:    isComputed,
			ForceNew:    !isUpdatable,
			Default:     providerschema.ConditionalDefault(!isComputed, string(api.DNSClusterFirst)),
		...
		"host_ipc": {
			...
			Computed:    isComputed,
			ForceNew:    !isUpdatable,
			Default:     providerschema.ConditionalDefault(!isComputed, false),
		...
		"restart_policy": {
			...
			Computed:    isComputed,
			ForceNew:    !isUpdatable,
			Default:     providerschema.ConditionalDefault(!isComputed, string(api.RestartPolicyAlways)),
		...
		"termination_grace_period_seconds": {
			...
			Computed:     isComputed,
			ForceNew:     !isUpdatable,
			Default:      providerschema.ConditionalDefault(!isComputed, 30),
			ValidateFunc: validators.ValidateTerminationGracePeriodSeconds,
			...
			Elem: &schema.Resource{
				Schema: map[string]*schema.Schema{
					...
					"label_selector": {
						...
						Optional:    true,
						Elem: &schema.Resource{
							Schema: providerschema.LabelSelectorFields(true),
						},
					},
				},
			},
					...
					Elem: &schema.Resource{
						Schema: map[string]*schema.Schema{
							...
							"mode": {
								...
								ValidateFunc: validators.ValidateModeBits,
							},
							"path": {
								Type:         schema.TypeString,
								Optional:     true,
								ValidateFunc: validators.ValidateAttributeValueDoesNotContain(".."),
		...
		Elem: &schema.Resource{
			Schema: map[string]*schema.Schema{
				"medium": {
					...
					Default:      "",
					ForceNew:     !isUpdatable,
					ValidateFunc: validators.ValidateAttributeValueIsIn([]string{"", "Memory"}),
				},
				"size_limit": {
					Type:             schema.TypeString,
								...
								Elem: &schema.Resource{
									Schema: map[string]*schema.Schema{
										...
										"expiration_seconds": {
											...
											ValidateFunc: validators.ValidateIntGreaterThan(600),
										},
										"path": {
											Type:        schema.TypeString,
