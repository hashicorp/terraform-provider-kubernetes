...
func TestKubernetesManifest_WaitForFields_Pod(t *testing.T) {
	...
	tfconfig := loadTerraformConfig(t, "WaitFor/wait_for_fields_manifest_pod.tf", tfvars)
...
func TestKubernetesResource_WaitForFields_Pod(t *testing.T) {
	ctx := context.Background()
	...
	k8shelper.CreatePod(t, name, namespace)
	defer k8shelper.DeleteNamespacedResource(t, name, namespace, kubernetes.NewGroupVersionResource("v1", "pods"))

	tfvars := TFVARS{
		"namespace": namespace,
		"name":      name,
	}
	tfconfig := loadTerraformConfig(t, "WaitFor/wait_for_fields_resource_pod.tf", tfvars)
	tf.RequireSetConfig(t, tfconfig)
	tf.RequireInit(t)
	...
	tfstate := tfstatehelper.NewHelper(tf.RequireState(t))
	tfstate.AssertAttributeValues(t, tfstatehelper.AttributeValues{
		"data.kubernetes_resource.test.wait_for": map[string]interface{}{
			"fields": map[string]interface{}{
				"status.containerStatuses[0].ready":        "true",
				"status.containerStatuses[0].restartCount": "0",
