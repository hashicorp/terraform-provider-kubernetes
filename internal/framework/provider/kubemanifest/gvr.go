...
// DecodeYAML parses a single-document YAML (or JSON — YAML is a superset) string into
// an unstructured object. Multi-document input is rejected: these resources manage
// exactly one object.
func DecodeYAML(y string) (*unstructured.Unstructured, error) {
	if n, err := CountYAMLDocuments(y); err != nil {
		return nil, err
	} else if n > 1 {
		return nil, fmt.Errorf("yaml_body contains %d documents; this resource manages exactly one object. "+
...
// A DeferredDiscoveryRESTMapper is used so a CRD registered earlier in the same apply
// can be discovered on retry (RFC-011 §6.2).
...
// ResourceInterface returns a (optionally namespaced) dynamic client for the GVR.
func ResourceInterface(dyn dynamic.Interface, gvr schema.GroupVersionResource, namespaced bool, ns string) dynamic.ResourceInterface {
	if namespaced {
...
// UnstructuredForImport builds a minimal object from the parsed import identity so the
// GVR can be resolved and a Get performed.
