...
func (p *KubernetesProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	//
	// Setting this once is enough: the framework server replays it for plan, read, import
	// and data source reads on our behalf.
	//
	// Guarded on DeferralAllowed — the framework rejects a deferred response the client
	// did not ask for, and that error path also skips handing the meta to resources.
